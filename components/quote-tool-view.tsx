...
function CountrySearchField({
  ...
  return (
    <SearchPicker
      choices={countries.map((country) => ({ id: country, label: country }))}
      label="Quote country"
...
export function QuoteToolView() {
  ...
  return (
    <PageFrame
      mainClassName="flex flex-1 flex-col gap-4 p-4 [--mobile-page-bottom-padding:calc(10rem+env(safe-area-inset-bottom,0px))] lg:p-6"
      headerPlacement="inset"
      header={<SiteHeader title="Quote Tool" />}
    >
      <Card className="rounded-lg shadow-sm">
        <CardContent className="relative">
          ...
          <Table
            containerClassName="mt-6 hidden rounded-lg border md:block"
            className="min-w-[640px] [&_td]:whitespace-normal [&_th]:h-auto [&_th]:whitespace-normal"
          >
            <TableHeader className="bg-muted/50 text-muted-foreground">
              <TableRow className="border-b">
                ...
                {showBulkUnits ? (
                  <TableHead className="w-32 px-3 py-2 text-left font-medium">
                    Bulk Units
                  </TableHead>
                ) : null}
                <TableHead className="w-32 px-3 py-2 text-right font-medium">
                  Unit Price
                </TableHead>
                <TableHead className="w-36 px-3 py-2 text-center font-medium">
                  Quantity
                </TableHead>
              </TableRow>
            </TableHeader>
                      ...
                      ? section.items.map((item) => {
                          ...
                          return (
                            <TableRow
                              ...
                              className="border-b border-l-2 border-l-sky-400 transition-colors outline-none last:border-b-0 focus-visible:bg-accent/60 dark:border-l-sky-500"
                              onKeyDown={(event) =>
              ...
              {primaryItems.map((item) => {
                ...
                return (
                  <TableRow
                    ...
                    tabIndex={0}
                    aria-label={`${item.name}, quantity ${quantity}`}
                    className="border-b transition-colors outline-none last:border-0 focus-visible:bg-accent/60"
                    onKeyDown={(event) => handleDesktopItemKeyDown(event, item)}
                  >
                    <TableCell className="px-3 py-3 align-top">
                      <div className="font-medium">{item.name}</div>
                    </TableCell>
        ...
        {primaryItems.map((item) => {
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="leading-snug font-medium">{item.name}</div>
                </div>
                <div className="shrink-0 text-right font-medium">
                  {formatMoney(unitPrice)}
                </div>
